...
void md5_string(const char* data, char digest[33])
{
    ...
    MD5Update(&md5c, (unsigned char *)data, strlen(data));
...
{
    MD5_CTX md5c;
    unsigned char md5_val[16], buff[1024];
    FILE* fp;
    int i, ret;

    // 若要保证全平台使用则需要使用fopen
    fp = fopen(pathname, "rb");/* 二进制b 读r */
    ...
    /* 循环读取全文件内容 */
    while(ret = fread(buff, 1, sizeof(buff), fp) > 0)
    {
        MD5Update(&md5c, (unsigned char *)buff, ret);
    }
    /* 读取完了关闭文件 */
    fclose(fp);
